...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(40, 10, utf8_decode('Clave Adm'), 1, 1, 'C', 1); // Ancho reducido
    }
...
$sql = "SELECT id_nombre, NOMBRE, CORREO, CONTRASEÑA, CLAVE_ADM FROM administradores";
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        $pdf->Cell(40, 10, utf8_decode($row['CLAVE_ADM']), 1, 1, 'C', 0); // Ancho reducido
    }
} else {
    $pdf->Cell(0, 10, utf8_decode('No se encontraron registros.'), 1, 1, 'C', 0);
}

$pdf->Output('Reporte_Administradores.pdf', 'I'); // Mostrar el PDF en el navegador
?>
